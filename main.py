...
#ingilizce isimlendirme
yas_grubu=["22-25", "18-21", "26-"]
yas_araliklari = df['1'].value_counts()

# almak istenen sonuca göre detaylı görselleştirme araçlarını  incele (chart tipi)



# Yas aralıklarına göre kişi sayısı Sonuçları görüntüleyin
print(yas_araliklari)
plt.figure(figsize=(12,6))
sns.barplot(x=yas_grubu,y=yas_araliklari,data=yas_araliklari,palette='viridis')
plt.title("Yas gruplarına göre öğrenci sayisi")
plt.xlabel("Yas aralıkları")
plt.ylabel("Kisi sayısı")
plt.show()

# Cinsiyete göre kişi sayısı Sonuçları görüntüleyin

cinsiyete_gore_dagilim=df["2"].value_counts()
plt.figure(figsize=(12,6))
sns.barplot(x=["Erkek","Kadın"],y=cinsiyete_gore_dagilim,data=cinsiyete_gore_dagilim,palette='viridis')
...
# Her Yas grubuna göre cinsiyet sayısı Sonuçları görüntüleyin

for yas_grubu in yas_araliklari.index:
    yasli_veri = df[df['1'] == yas_grubu]
    cinsiyet_dagilimi = yasli_veri["2"].value_counts()
    print(f'{yas_grubu} Yaş Grubu Cinsiyet Dağılımı:')
    print(cinsiyet_dagilimi)
    print("\n")
    plt.figure(figsize=(12, 6))
    sns.barplot(x=cinsiyet_dagilimi, y=["Erkek","Kadın"], palette="viridis")
    plt.title(f'{yas_grubu.numerator} Yaş Grubu Cinsiyet Dağılımı')
    plt.xlabel("Cinsiyet")
    plt.ylabel("Kişi Sayısı")
    plt.show()

#Öğrencilerin ne kadarı hangi tür liseden mezun
lise_grup_ad=["Devlet","Özel","Diğer"]
lise_grup=df['3'].value_counts()
print(lise_grup)
plt.figure(figsize=(12,6))
sns.barplot(x=lise_grup_ad,y=lise_grup,data=lise_grup,palette="viridis")
plt.xlabel("Lise türü")
plt.ylabel("Kişi sayısı")
plt.show()

burslu_kisi=df["4"].value_counts()
burs_miktar=["50","75","100","25","0"]
sns.barplot(x=burs_miktar,y=burslu_kisi,data=burslu_kisi,palette='viridis')
